...
//  get value from input fields in index.php and set it as an variable to post it
    $treatment = mysqli_escape_string($db, htmlentities($_POST['treatments']));
    $worker = mysqli_escape_string($db, htmlentities($_POST['workers']));
    ...
    $phone = mysqli_escape_string($db, htmlentities($_POST['phone']));
    $birth = mysqli_escape_string($db, htmlentities($_POST['birthdate']));
...
//  query that post the abovve made variables in the right table and fields in the database
    ...
    $date = mysqli_escape_string($db, htmlentities($_POST['date1']));
    $startTime = mysqli_escape_string($db, htmlentities($_POST['time-start1']));
    $endTime = mysqli_escape_string($db, htmlentities($_POST['time-end1']));

    $date2 = mysqli_escape_string($db, htmlentities($_POST['date2']));
    $startTime2 = mysqli_escape_string($db, htmlentities($_POST['time-start2']));
    $endTime2 = mysqli_escape_string($db, htmlentities($_POST['time-end2']));

    $date3 = mysqli_escape_string($db, htmlentities($_POST['date3']));
    $startTime3 = mysqli_escape_string($db, htmlentities($_POST['time-start3']));
    $endTime3 = mysqli_escape_string($db, htmlentities($_POST['time-end3']));
